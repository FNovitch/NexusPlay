...
const menu: Array<{ key: AdminSection; icon: LucideIcon; label: string; description: string }> = [
  ...
  { key: "artesaos", icon: Store, label: "Lojas", description: "Cadastros, aprovações e status de loja." },
  { key: "produtos", icon: Boxes, label: "Produtos", description: "Catálogo, estoque e aprovação." },
  { key: "pedidos", icon: ClipboardList, label: "Pedidos", description: "Acompanhamento e troca de status." },
  { key: "avaliações", icon: MessageSquare, label: "Avaliações", description: "Moderação de comentários e notas." },
  { key: "categorias", icon: FolderTree, label: "Categorias", description: "Organização do catálogo." },
  { key: "assinaturas", icon: CreditCard, label: "Assinaturas", description: "Planos dos vendedores." },
  { key: "repasses", icon: WalletCards, label: "Repasses", description: "Valores liberados e pagamentos." },
  { key: "pagamentos", icon: ShieldCheck, label: "Pagamentos", description: "Histórico financeiro da plataforma." },
...
const filterConfig: Partial<Record<AdminSection, FilterDefinition[]>> = {
  ...
  produtos: [
    ...
    { key: "sellerId", label: "Loja", type: "search", placeholder: "ID da loja ou vendedor" }
  ],
  pedidos: [
...
export function AdminDashboard() {
  ...
  return (
    <main className="min-h-[75vh] bg-nexus-background">
    ...
    <aside className="lg:sticky lg:top-24 lg:h-max">
      <div className="panel p-3">
        <div className="mb-3 hidden px-3 py-2 lg:block">
          ...
          <strong className="text-nexus-contrast">{userName}</strong>
        </div>
        <nav className="flex gap-2 overflow-x-auto pb-1 lg:grid lg:overflow-visible lg:pb-0">
          {menu.map(({ key, icon: Icon, label }) => (
            <Link key={key} to={`/admin/${key}`} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-lg px-3 text-sm font-bold transition ${section === key ? "bg-nexus-primary text-white shadow-sm" : "text-nexus-muted hover:bg-nexus-primary/10 hover:text-nexus-primary"}`}>
              <Icon className="h-4 w-4" /> {label}
            </Link>
          ))}
          <button onClick={onLogout} className="flex min-h-11 shrink-0 items-center gap-2 rounded-lg px-3 text-sm font-bold text-nexus-secondary transition hover:bg-nexus-secondary/10 lg:mt-2">
...
function AdminToolbar({ title, description, section, detailId }: { title: string; description: string; section: AdminSection; detailId?: string }) {
  return (
    <header className="mb-5 flex flex-col gap-2 rounded-lg border border-nexus-line bg-nexus-surface p-5 shadow-soft">
      <p className="text-xs font-semibold uppercase tracking-[0.1em] text-nexus-muted">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {config.map((filter) => (
            <label key={filter.key} className="grid gap-1.5">
              <span className="text-xs font-semibold uppercase tracking-[0.08em] text-nexus-muted">{filter.label}</span>
              {filter.type === "select" ? (
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {section === "clientes" && (
        <>
          ...
          <button className="btn-secondary px-3 py-1 text-xs text-nexus-secondary" onClick={() => openAction({ title: "Desativar cliente", description: "Esta ação desativa o cliente e o usuário vinculado.", path: `/admin/clientes/${item.id}`, method: "delete" })}>Desativar</button>
        </>
      )}
      {section === "artesaos" && (
        <>
          {!isApproved && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Aprovar Loja", description: "A loja será liberada para operar.", path: `/admin/artesaos/${item.id}/aprovar`, method: "put" })}>Aprovar</button>}
          {!isRejected && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Recusar Loja", description: "Informe o motivo da recusa para orientar o vendedor.", path: `/admin/artesaos/${item.id}/recusar`, method: "put", reasonRequired: true })}>Recusar</button>}
          {!isBlocked && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Bloquear Loja", description: "Informe o motivo do bloqueio.", path: `/admin/artesaos/${item.id}/bloquear`, method: "put", reasonRequired: true })}>Bloquear</button>}
          {isBlocked && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Desbloquear Loja", description: "A loja volta a operar conforme o status atual.", path: `/admin/artesaos/${item.id}/desbloquear`, method: "put" })}>Desbloquear</button>}
        </>
      )}
      {section === "produtos" && (
        <>
          {item.status !== "ACTIVE" && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Aprovar produto", description: "O produto ficará ativo na loja.", path: `/admin/produtos/${item.id}/aprovar`, method: "put" })}>Aprovar</button>}
      ...
      {section === "pedidos" && (
        <label className="grid gap-1 text-xs font-bold text-nexus-muted">
          Alterar status
      ...
      {section === "avaliações" && (
        <>
          {!isHidden && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Ocultar avaliação", description: "A avaliação deixará de aparecer publicamente.", path: `/admin/avaliações/${item.id}/ocultar`, method: "put" })}>Ocultar</button>}
          {isHidden && <button className="btn-secondary px-3 py-1 text-xs" onClick={() => openAction({ title: "Exibir avaliação", description: "A avaliação voltará a ficar visível.", path: `/admin/avaliações/${item.id}/exibir`, method: "put" })}>Exibir</button>}
        </>
      )}
      {section === "categorias" && (
        <>
          ...
          <button className="btn-secondary px-3 py-1 text-xs text-nexus-secondary" onClick={() => openAction({ title: "Excluir Categoria", description: "Se houver produtos vinculados, a categoria será desativada.", path: `/admin/categorias/${item.id}`, method: "delete" })}>Excluir</button>
        </>
      )}
      {section === "assinaturas" && (
        <>
    ...
    <article className="panel p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <strong className="line-clamp-2 text-nexus-contrast">{primaryLabel(item)}</strong>
        </div>
        {item.status && <StatusBadge value={item.status} />}
      </div>
      <dl className="grid gap-2 text-sm">
        {columns.slice(0, 4).map((column) => (
          <div key={column.key} className="flex justify-between gap-3 border-t border-nexus-line/60 pt-2 first:border-t-0 first:pt-0">
            <dt className="font-bold text-nexus-muted">{column.label}</dt>
            <dd className="text-right text-nexus-contrast">{column.render ? column.render(item) : displayValue(item, column.key)}</dd>
          </div>
        ))}
      </dl>
      <div className="mt-4">
  ...
  return (
    <footer className="panel flex flex-col gap-4 p-4 xl:flex-row xl:items-center xl:justify-between">
      <div className="grid gap-2">
        <p className="text-sm font-bold text-nexus-muted">
          {total > 0 ? `Exibindo ${from}-${to} de ${total} itens` : "Sem itens para exibir"} · Página {page} de {totalPages}
        </p>
        <label className="flex items-center gap-2 text-sm font-bold text-nexus-muted">
          Itens por página
        ...
        <div className="col-span-4 hidden gap-2 md:flex">
          {pages.map((pageNumber) => (
            <button
              key={pageNumber}
              type="button"
              className={`btn-secondary h-10 min-w-10 px-3 text-xs ${pageNumber === page ? "bg-nexus-primary text-white" : ""}`}
...
function AdminDetailPanel({ section, item, loading, backTo, openAction, onStatusChange }: { section: AdminSection; item: any; loading: boolean; backTo: string; openAction: (action: ModalAction) => void; onStatusChange: (item: any, status: string) => void }) {
  ...
  return (
    <section className="panel overflow-hidden">
      <div className="border-b border-nexus-line bg-nexus-surface/80 p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="eyebrow">Detalhe</p>
            <h2 className="mt-1 text-2xl font-semibold text-nexus-contrast">{primaryLabel(item)}</h2>
    ...
    return (
      <>
        <div className="grid gap-4">
          <img src={productImageUrl(item)} alt={item.name ?? "Produto"} className="aspect-[4/3] w-full rounded-lg object-cover" loading="lazy" decoding="async" onError={handleImageError} />
          {images.length > 1 && (
            <div className="grid grid-cols-4 gap-2">
              {images.slice(0, 8).map((image, index) => <img key={`${image}-${index}`} src={image} alt="" className="aspect-square rounded-lg object-cover" loading="lazy" decoding="async" onError={handleImageError} />)}
            </div>
          )}
          <InfoGrid items={[["Status", <StatusBadge value={item.status} />], ["Categoria", item.category?.name], ["Loja", item.seller?.storeName], ["Preço", currency.format(Number(item.price ?? 0))], ["Estoque", item.stock], ["Peso", `${Number(item.weight ?? 0)} kg`], ["Favoritos", item._count?.favorites ?? 0], ["Pedidos", item._count?.orderItems ?? item.salesCount ?? 0], ["Avaliações", item._count?.reviews ?? item.totalReviews ?? 0]]} />
        </div>
        <div className="grid gap-4">
  ...
  return (
    <dl className="grid gap-3 sm:grid-cols-2">
      {items.map(([label, value]) => (
        <div key={label} className="rounded-lg border border-nexus-line bg-nexus-paper p-4">
          <dt className="text-xs font-semibold uppercase tracking-[0.08em] text-nexus-muted">{label}</dt>
          <dd className="mt-2 font-semibold text-nexus-contrast">{value ?? "Não informado"}</dd>
        </div>
      ))}
    </dl>
  );
...
function DetailBox({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="rounded-lg border border-nexus-line bg-nexus-paper p-4"><h3 className="mb-3 text-lg font-semibold text-nexus-contrast">{title}</h3><div className="text-sm leading-6 text-nexus-muted">{children}</div></section>;
}
...
function AddressList({ addresses }: { addresses: any[] }) {
  ...
  return (
    <div className="mt-3 grid gap-2">
      {addresses.map((address) => <p key={address.id} className="rounded-lg bg-nexus-surface p-3">{address.street}, {address.number} · {address.city}/{address.state}</p>)}
    </div>
  );
}
...
function InfoList({ items }: { items: Array<[string, React.ReactNode]> }) {
  return (
    <dl className="grid gap-2">
      {items.map(([label, value]) => (
        <div key={label} className="flex justify-between gap-3 border-t border-nexus-line/60 pt-2 first:border-t-0 first:pt-0">
          <dt className="font-bold text-nexus-contrast">{label}</dt>
...
function OrderItems({ items }: { items: any[] }) {
  ...
  return (
    <div className="grid gap-3">
      {items.map((orderItem) => (
        <div key={orderItem.id} className="rounded-lg border border-nexus-line p-3">
          <strong className="text-nexus-contrast">{orderItem.productName || orderItem.product?.name}</strong>
          <p className="text-sm text-nexus-muted">{orderItem.seller?.storeName} · {orderItem.quantity} un. · {currency.format(Number(orderItem.total ?? 0))}</p>
          {orderItem.customizationNotes && <p className="mt-2 text-xs text-nexus-muted">Personalização: {orderItem.customizationNotes}</p>}
        </div>
      ))}
    </div>
  );
...
function Timeline({ items }: { items: any[] }) {
  ...
  return (
    <ol className="grid gap-3">
      {items.map((entry) => (
        <li key={entry.id} className="rounded-lg border border-nexus-line p-3">
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge value={entry.newStatus} />
            <span className="text-xs text-nexus-muted">{formatDate(entry.createdAt)}</span>
          </div>
...
function PaymentList({ items }: { items: any[] }) {
  ...
  return (
    <div className="grid gap-2">
      {items.map((payment) => (
        <div key={payment.id} className="rounded-lg border border-nexus-line p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <StatusBadge value={payment.status} />
            <strong className="text-nexus-contrast">{currency.format(Number(payment.amount ?? 0))}</strong>
          </div>
          <p className="mt-1 text-xs text-nexus-muted">{payment.description ?? payment.mpPaymentId ?? "Pagamento sem descrição"} · {formatDate(payment.createdAt)}</p>
        </div>
      ))}
    </div>
  );
...
function ShippingList({ items }: { items: any[] }) {
  ...
  return (
    <div className="grid gap-2">
      {items.map((shipping) => (
        <p key={shipping.id} className="rounded-lg border border-nexus-line p-3">
          <strong className="text-nexus-contrast">{shipping.carrier} · {shipping.service}</strong>
          <span className="block text-xs text-nexus-muted">{currency.format(Number(shipping.price ?? 0))} · {shipping.deliveryTime} dias · {shipping.trackingCode || "sem rastreio"}</span>
        </p>
      ))}
    </div>
  );
...
function PayoutList({ items }: { items: any[] }) {
  ...
  return (
    <div className="mt-3 grid gap-2">
      {items.map((payout) => (
        <p key={payout.id} className="rounded-lg border border-nexus-line p-3">
          <strong className="text-nexus-contrast">{payout.artisan?.storeName ?? payout.artisan?.user?.email}</strong>
          <span className="block text-xs text-nexus-muted">{statusLabel(payout.status)} · {currency.format(Number(payout.availableAmount ?? 0))}</span>
        </p>
      ))}
    </div>
  );
  ...
  return (
    <div className="grid gap-2">
      {items.map((subscription) => (
        <p key={subscription.id} className="rounded-lg border border-nexus-line p-3">
          <strong className="text-nexus-contrast">{subscription.plan?.name ?? "Plano não informado"}</strong>
          <span className="block text-xs text-nexus-muted">{statusLabel(subscription.status)} · expira em {formatDate(subscription.expirationDate)}</span>
        </p>
      ))}
    </div>
  );
...
function ProductMiniList({ items }: { items: any[] }) {
  ...
  return (
    <div className="grid gap-2">
      {items.map((product) => (
        <div key={product.id} className="flex items-center gap-3 rounded-lg border border-nexus-line p-3">
          <img src={productImageUrl(product)} alt="" className="h-12 w-12 rounded-lg object-cover" loading="lazy" decoding="async" onError={handleImageError} />
...
function OrderMiniList({ items }: { items: any[] }) {
  ...
  return (
    <div className="grid gap-2">
      {items.map((order) => (
        <Link key={order.id} to={`/admin/pedidos/${order.id}`} className="rounded-lg border border-nexus-line p-3 transition hover:border-nexus-primary">
          <strong className="text-nexus-contrast">{order.orderCode ?? order.id}</strong>
          <span className="block text-xs text-nexus-muted">{statusLabel(order.status)} · {currency.format(Number(order.total ?? 0))} · {formatDate(order.createdAt)}</span>
        </Link>
      ))}
    </div>
  );
...
function ReviewList({ reviews }: { reviews: any[] }) {
  ...
  return (
    <div className="grid gap-2">
      {reviews.map((review) => (
        <div key={review.id} className="rounded-lg border border-nexus-line p-3">
          <strong className="text-nexus-contrast">{review.rating} estrelas</strong>
          <p className="mt-1 line-clamp-3">{review.comment}</p>
          <span className="text-xs text-nexus-muted">{review.author?.name ?? review.author?.email ?? "Cliente"} · {formatDate(review.createdAt)}</span>
        </div>
      ))}
    </div>
  );
...
function DashboardView({ data, loading, onRetry }: { data: any; loading: boolean; onRetry: () => void }) {
  ...
  const cards = [
    ...
    ["Lojas", metrics.totalArtesaos],
    ["Lojas Pendentes", metrics.artesaosPendentes],
  ...
  return (
    <div className="grid gap-5">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map(([label, value]) => <div key={label} className="panel p-5"><PackageCheck className="mb-3 h-5 w-5 text-nexus-primary" /><strong className="block text-2xl text-nexus-contrast">{String(value ?? 0)}</strong><span className="text-sm text-nexus-muted">{label}</span></div>)}
      </div>
      <div className="grid gap-5 lg:grid-cols-2">
        <MiniList title="Últimos Pedidos" items={data?.ultimosPedidos ?? []} empty="Sem pedidos recentes." />
        <MiniList title="Lojas Pendentes" items={data?.artesaosPendentesLista ?? []} empty="Sem pendências agora." />
      </div>
...
function MiniList({ title, items, empty }: { title: string; items: any[]; empty: string }) {
  return <section className="panel p-5"><h2 className="mb-3 text-xl font-semibold text-nexus-contrast">{title}</h2>{items.length === 0 ? <p className="text-nexus-muted">{empty}</p> : items.map((item) => <div key={item.id} className="border-t border-nexus-line py-3 first:border-t-0"><strong className="text-nexus-contrast">{item.storeName ?? item.orderCode ?? item.id}</strong><p className="text-sm text-nexus-muted">{item.status ?? item.user?.email ?? item.buyer?.email}</p></div>)}</section>;
}
...
function SettingsView({ userName }: { userName: string }) {
  return (
    <div className="panel p-5">
      ...
      <h2 className="mt-2 text-2xl font-semibold text-nexus-contrast">{userName}</h2>
      <p className="mt-3 max-w-2xl text-sm leading-6 text-nexus-muted">Use os endpoints de perfil e alteração de senha para evoluir esta área. A tela já reserva espaço para preferências, segurança e dados do administrador.</p>
    </div>
  );
}
...
function LoadingState({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`panel grid gap-3 p-5 ${compact ? "" : "min-h-56"}`}>
      {[0, 1, 2].map((item) => <div key={item} className="h-12 animate-pulse rounded-lg bg-nexus-line/50" />)}
    </div>
  );
}
...
function EmptyAdminState({ onClearFilters }: { onClearFilters: () => void }) {
  return (
    <div className="panel grid min-h-56 place-items-center p-6 text-center">
      <div>
        <PackageCheck className="mx-auto mb-3 h-8 w-8 text-nexus-primary" />
        <h2 className="text-xl font-semibold text-nexus-contrast">Nenhum Registro Encontrado</h2>
        <p className="mt-2 text-sm text-nexus-muted">Ajuste os filtros ou limpe a busca para ver mais resultados.</p>
...
function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="panel flex flex-col gap-3 p-5 text-nexus-muted sm:flex-row sm:items-center sm:justify-between">
...
function ActionModal({ action, reason, onReasonChange, onClose, onConfirm }: { action: ModalAction; reason: string; onReasonChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-nexus-contrast/40 p-4 backdrop-blur-sm">
      <section className="panel w-full max-w-lg p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="eyebrow">Confirmar Ação</p>
            <h2 className="mt-2 text-2xl font-semibold text-nexus-contrast">{action.title}</h2>
          </div>
          <button className="btn-icon" onClick={onClose} aria-label="Fechar"><X className="h-5 w-5" /></button>
        </div>
        <p className="mt-3 text-sm leading-6 text-nexus-muted">{action.description}</p>
        {action.reasonRequired && (
          <label className="mt-4 block">
            <span className="mb-2 block text-sm font-medium text-nexus-contrast">{action.reasonLabel ?? "Motivo Obrigatório"}</span>
...
function columnsFor(section: AdminSection): Column[] {
  ...
  const map: Partial<Record<AdminSection, Column[]>> = {
    ...
    produtos: [
      ...
      { key: "seller", label: "Loja", render: (item) => item.seller?.storeName ?? "Não informado" },
      statusColumn(),
    ...
    assinaturas: [
      statusColumn(),
      { key: "artisan", label: "Loja", render: (item) => item.artisan?.storeName ?? item.artisan?.user?.email },
...
function NameCell({ title, subtitle }: { title?: string; subtitle?: string }) {
  return <div><strong className="block text-nexus-contrast">{title ?? "Sem nome"}</strong>{subtitle && <span className="text-xs text-nexus-muted">{subtitle}</span>}</div>;
}

function ProductCell({ item }: { item: any }) {
  return <div className="flex items-center gap-3"><img src={productImageUrl(item)} className="h-12 w-12 rounded-lg object-cover" alt="" loading="lazy" decoding="async" onError={handleImageError} /><NameCell title={item.name} subtitle={item.category?.name} /></div>;
}
...
function StatusBadge({ value }: { value?: string | boolean | null }) {
  ...
  const tone = positive ? "bg-nexus-primary/10 text-nexus-primary" : warning ? "bg-nexus-secondary/10 text-nexus-secondary" : "bg-red-50 text-red-700";
...
function labelForFilter(key: string) {
  const labels: Record<string, string> = { q: "Busca", status: "Status", blocked: "Bloqueio", rating: "Nota", productId: "Produto", customerId: "Cliente", categoryId: "Categoria", sellerId: "Loja", paymentStatus: "Pagamento", dateFrom: "De", dateTo: "Até", active: "Status", type: "Tipo" };
